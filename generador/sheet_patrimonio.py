...
def build(wb, A, C, T):
    ...
    liq_ag = r
    # El patrimonio líquido del año anterior se puede llegar por dos caminos, y
    # el libro deja los dos abiertos porque no siempre se tiene el mismo dato a
    # la mano: o se digita la casilla 31 del 210 presentado, o se arma con el
    # patrimonio bruto —que la exógena sí informa— menos los pasivos que
    # suministre el contribuyente. La casilla del líquido viene calculada por
    # el segundo camino pero es editable: quien tenga el formulario delante
    # escribe la casilla 31 encima y listo.
    ...
    bruto_ant = r
    A['pat_bruto_anterior'] = bruto_ant
    ws[f'H{bruto_ant}'].comment = Comment(
        f'NOTA 10 — Patrimonio bruto del renglón 29 de la declaración del año\n'
        f'gravable {ant}. Viene prediligenciado con lo que informa la exógena\n'
        f'({fmt_pesos(C["patrimonio_bruto_anterior"])}); confírmelo contra la declaración presentada.',
        AUT, 320, 90)
    r = s.datarow(h=20.5)
    s.txt(r, 'B', f'(−) Pasivos Declarados {ant}  ·  renglón 30', 'G', sz=10.0, v='center')
    s.mark_input(r, 'H')
    s.money(r, 'H', None, sz=10.0)
    pas_ant = r
    A['pasivos_anterior'] = pas_ant
    ws[f'H{pas_ant}'].comment = Comment(
        f'NOTA 10 — Deudas del renglón 30 de la declaración del año gravable\n'
        f'{ant}. La exógena NO las informa: este dato lo suministra el\n'
        f'contribuyente. Mientras siga vacía, la variación patrimonial se\n'
        f'calcula contra el patrimonio bruto y sale sobrestimada.', AUT, 330, 100)
    r = s.datarow(zebra=True, h=20.5)
    s.txt(r, 'B', f'(=) Patrimonio Líquido Declarado {ant}  ·  casilla 31 del Formulario 210',
          'G', sz=10.0, b=True, v='center')
    s.mark_input(r, 'H')
    s.money(r, 'H', f'=H{bruto_ant}-N(H{pas_ant})', sz=10.0, b=True)
    liq_ant = r
    A['pat_liq_anterior'] = liq_ant
    ws[f'H{liq_ant}'].comment = Comment(
        f'NOTA 10 — Es la casilla 31 del formulario 210 de {ant}: patrimonio\n'
        f'bruto menos pasivos, que es como la calcula el propio formulario.\n'
        f'Viene con esa resta hecha, pero la casilla es editable: si tiene la\n'
        f'declaración delante, escriba la casilla 31 encima y las dos filas de\n'
        f'arriba quedan como simple referencia.', AUT, 340, 110)
    ...
    A['pat_dif'] = r
    s.note(f'NOTA 10 — La comparación patrimonial del art. 236 E.T. se hace sobre el patrimonio líquido: enfrenta el reconstruido para {ag} —bruto menos deudas— contra el declarado en {ant}. Del año anterior la exógena solo informa el patrimonio bruto, así que los pasivos de {ant} los suministra el contribuyente: mientras esa casilla siga vacía la variación sale sobrestimada. Si tiene el formulario 210 a la mano, puede saltarse las dos y escribir directamente la casilla 31. Lo que la variación no explique con la renta gravable, los ingresos no constitutivos y el impuesto pagado queda al final, en rojo: es renta líquida gravable por comparación patrimonial si no se justifica con la casilla de ajuste de capital.', h=38.0)
    s.gap()
