...
def construir_datos_py(caso, fuente_archivo):
    ...
    txt = PLANTILLA.format(
        ...
        texto_brecha=(f'El patrimonio bruto reconstruido desde la exógena es de {_pesos(caso["pat_bruto"])}; la exógena informa además {_pesos(caso["pat_anterior"])} de patrimonio bruto del año anterior, que sirve de referencia pero NO es lo que va en la conciliación. La comparación del art. 236 E.T. se hace sobre patrimonio líquido, así que hay tres casillas crema que cerrar en su orden: el patrimonio líquido de la casilla 31 del formulario 210 del año anterior —tomado de la declaración presentada, no del reporte—, el impuesto de renta efectivamente pagado en el año y, al final, el ajuste de capital. La renta líquida gravable y los ingresos no constitutivos ya entran solos. Lo que siga sin justificar debe explicarse antes de presentar: activos que no llegan por exógena (efectivo, muebles y enseres, préstamos a terceros, criptoactivos, inmuebles en otros municipios), rentas exentas, ganancias ocasionales, herencias o donaciones. Diligencie los grupos con los certificados y la diferencia se irá cerrando sola.'
