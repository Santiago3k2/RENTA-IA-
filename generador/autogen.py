...
def construir_datos_py(caso, fuente_archivo):
    ...
    txt = PLANTILLA.format(
        ...
        texto_brecha=(f'El patrimonio bruto reconstruido desde la exógena es de {_pesos(caso["pat_bruto"])} frente a {_pesos(caso["pat_anterior"])} declarados el año anterior. La comparación del art. 236 E.T. se hace sobre patrimonio líquido, así que hay tres casillas crema que cerrar en su orden: las deudas declaradas el año anterior —la exógena no las informa y sin ellas la variación sale sobrestimada—, el impuesto de renta efectivamente pagado en el año y, al final, el ajuste de capital. La renta líquida gravable y los ingresos no constitutivos ya entran solos. Lo que siga sin justificar debe explicarse antes de presentar: activos que no llegan por exógena (efectivo, muebles y enseres, préstamos a terceros, criptoactivos, inmuebles en otros municipios), rentas exentas, ganancias ocasionales, herencias o donaciones. Diligencie los grupos con los certificados y la diferencia se irá cerrando sola.'
